/*
 ...
 - 아래의 Past 클래스는 항상 true를 반환하는 onSetData 메서드를 가지고 있다.
 - Past 클래스를 작성하는 시점에서는 아무런 의미가 없는 메서드처럼 보인다.
 - 그러나 누군가가 이 메서드를 재정의해 활용할 가능성이 있겠다는 생각이 든 Past 개발자는 이 불필요한 호출 코드를 추가함으로써 미래와의 대화를 시도한 것이라고 할 수 있다.
